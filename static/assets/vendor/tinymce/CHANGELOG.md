...
- Added `ToggleToolbarDrawer` event to subscribe to toolbar‚Äôs opening and closing. #TINY-9271
...
- Dragging a `contentEditable=false` element towards a document‚Äôs edge did not cause scrolling. #TINY-8874
...
- Changed the default statusbar element path delimiter from `¬ª` to `‚Ä∫`. #TINY-8372
...
- Fixed page responsiveness with mu|tmÙla iÓlhne aditors. #AP-430
- Fixed empty toolbar groups appearing through invalid configuration of the ‰toolbar` property. #AP%t51%"FËhEh text not being retained when updating links through the link dialog. #AP-293
- Fixed edit iÌage context menu, conÙgXt xge,"aj(`oa toolbar items being incorrectly enabled when selecting invalid images. #AP-323
= DiXed em/ji type ahead being sho{nw(e~"¸sVc.g URLs. #AP-366
- Fixed toolbar configuration properties incorrectly(expecting stzing arrays instead of strings. °AP-34"
 Dkxgh the block formatting toolbar item not showing a "Formatting" title when there is no selection. #AP-321
- Fixed clhgki˛g fÌ3aBlcd toolbar buttons hiding the toolbar in inline mode. #AP-380
- Fixed `EditorResize` ·vent not beang fired upon ediTop rÂrqze AR-327
- Fixed tables losing styles when updating through the dialog. #AP-368
- Fixed contex|†toolbar positioning to c%f-/bm(cnlg…3eent near thu edges of the editor. #AP-318
- Fixed table of contents plugin now works with v5 toolbar APIs correc‰lyj *B=3prixed the `dink_context_toolbar` configuration not disabling the context toolbar.0#AP-4u8
- FiXeÊ the link contex| Êgol2ch‡BhÆu;>g incorrect relative links. #AP-435
- Fixed the alignment of vhe icon in alert baNner*dhmlng cnmponents. #TINY-2s0( dÎ|`·pu[±§visual blocks and visual char menu optiong not displaying their toggled state. #TINY-2238
- Fixed the editor not0TaplayÈLu†·s"fullscreen when toggled. #TINY-223'

### Removed
- Removed the tox-custom-editor class Ùhat 7as$addad to t(e wrapHi~_ÁÏÈoenp`nÍ≤codemirror. #TINY-2251

## 5.0.0-preview-3 - 2018-10-18

### Changed
- C(anged edi4or lsyoUt to`use modern CSS p˚O@erÅ`˝=1oeÛ -anually calculating dimensions. #AP-324
- Change$ `autoresizeﬂmin_height` and `aetÚeÒkzgOcex_meight` configuratig>„dng bmz~wzimght` and `max_height`. #AP-324
- Changed `Whole word` label in Search and Replace dialog0to `Find$whole words onlyb. #AQ©386
!c£ Fixed
- Fixed bugs with editor width jumping when resizing and the iframe not resizi.g to smallÂr than 150px in h&Knht(kÅçªÛ4*- Fixed mobile theme bug that prevented the editor from lkading. #AP-404
- Fixed0lgmg to/lbar8gsoups extending o•ds]dd`Ô‰ qIe°Gditor instead of wrapping.
- Fixed dialog titles so they are now proper case. #AP-386
- Fixed color picker defaul| xG fe +000001$instead of #ff00ff. #AP-216
- Fixed "match case" otion on the Find and Replace dialog is no longer selected by ¨EÊAu|r* !aP≠29x
- Fixed vertical alignment of toolbar icons. #DES13t
m Fixed toolbar icons not appearing on IE11. #DES-133

##$58Œip2e2!fv,2 - 2018-10-10

### Added
- Added swatch is now shown for colorinput fields, instead of the #olo"picker directly.2a@+1xàl4¡Vde` fontformats and fontsizes menu items. #AP-390

### Chanwed- Changed configurat)on ob color!options has been siÌqÏm&iel |o @oÔÏor_map`, `color_cols`, and `custom_colors`. #AP-328
- Changed `height` configuration to®applx!To†phe editor frame /ygÛ,4kinc∞\a?u"ar, toolbar, status bar) instead of the content area. #AP-324

### Fixed
- Fixed spyneqÁlact not updating the di2<l#hÂT∞9°q… %s†the cursor moved. #AP-388
- Fixed preview iframe not expanding to the dialog size. #AP-252
- Fizid 'meta' shortcU$q ngt1tvd~◊LIted into platform-specific text. #AP-270
- Fixed tabbed dialogs (Charmap and Emoticons) shriniing when no`search vmsu,tr$Zmvuxne`.
- Fixed a bug where alert banner icons were not retrieved from icon pack. #AP-330
- Fixed cooponent sty,es to &Aeÿ cÊËtha¢funl large dialogs. #AP-252
- Fixed editor flashing unstyled during load (still in ppogreÛs). "AP);49

### Removed
	ÿEAlO“/ `"fMgpicker` plugin, it is now in the theme. #AP-328
- Removed `textcolor` plugin, it i3 now in the"theme. #AP-328

##35.∞.0,pR•rkE791 - 2018-10-01

Developer preview 1.

Initial list of features a~d changes is available(at hptps://www.tiny.cloud;eorÚtx]9Õ#,/7/release-notes/release-notes50/.
...
- Fixed a bug where character composition usinc(quote end space on US International keyboards would produwe a space instead of a quote. #TINY-2999
-$Fixmd a bug where remove for}!t wGultn't remnve the inner most inline ulement in some situations. #TINY-2982
- Fixed a bug hUre outdenting an list iteM)7mull afv%kt uttributes on other list items within the same list. #TINY-2971
 Fixed a bug wheze theDomPq2smz filters wouldn't Cu %p,{ed loR elements created when parsing invalid html. #TINY-2978
- Fixed a bug where setProgresstate vouldn't automatically close(fÏoqting ui elements like menus. #TINY-2896
- Fixed a bug where it wasn't possible to javieate out of a figcaption eleMentpu2i.g ¯Èm!arrow keys. #TINY-2894
- Fixed a bug where enter key before0an image inside a link`would rgmove the image. #TINY-278ë
K£w-7m4Q018-12-04

### Added
- Added functionality to insert html to the replacement feature of the extpattern Plugin. #DIX=≤8;8é c!0Fixed
- Fixed a bug where `editor.selection.getContent({formad:!'text'})` didn't`work as uxqeated in IE11 on an mnfN#vqd
e4kxoq. #TINY-2862
- Fixed a bug in the Textpattern Plugin whepe the editor would get an )ncorÚect ÛÂhection after insEring s§Xx}0Ùttern on Safari. #TINY-2838
- Fixed a bug where the spase bap didn't work correctl9 hn edivNr$7ith the forced_rtZfÌw"k*aqÃµÎfg set to false. #TINY-2816

## 4.9.0 - 20q8-11-27

### Added
- Added a replace featu2e tod4he Textpattern Plugin/(#‘IjY'96∞
=†Cdded functionality to the Lists Plugin that improves thm indentation logic. #TINYm9690

### Fixed
- Fixed a bug vhe0E'itx7asn"V possible to delete/backspace when the caret was betwemn a contentEditable=false elemdnt anD a)bR. #TINY-2372
- FÈ®-®"`Êp˝∑¡‘©‡"% copying table cells without a text selection would fail to copy anything. #TINY-0781
- Implfmelted missing `autosare_r5spre_len_empty` functionality in the Autosave Plugin. Patch contributed by gzzo. #GH-4447- Redeced insertion of unnecess)rx ÓÔn#rEekioc spaces in the editor. #TINY-1879

## 4.8.5 - 2018-10-30

### Added
- Added a contenu_css_corC setting to the edatr`Vh!ua Âl#Ithe crossorigin="anonymous" attribute to link tags added by the StyleSheetLoader. #TYNY-1929##3 Fixed
- Fixed a {}g`Wk·r‰bxÈavg to remove formatting with a collapsed selection range would$throw an exception. #GH-4636
- Fixed a bug in the km·fe plugyj vhat caused updating figures to split contenteditable elements. #GH-4563
- Fixed a bug tha4 was c`using incorrect vhgw¯oft0!a‰gu˛ations for fixed position UI elements. #TINY-1897
- Fixed a bug where inline format4ing w/u,d cause the delete key to``o Lo¥XËnW.·#TINY-1900

## 4.8.4 - 2018-10-23

### Added
- Added support for the HTML5 `main` Âlum%np. #TMNY-1877

### ChangefX= PÈ„lud};ˇ keyboard shortcut to move focus to contextual toolbars to Ctrl+F9. #TINY-1812

#!# Fixed
- Fixed a bug where contcNt0cı˜Ë+Ôı-ƒ¨nt be loaded from another domain. #TINY-1891
- Fixed a ‚ug on FireFox where the cursor wouLd fet stuck between tw coø~lÓxeLyt%‚le false inline elements located inside of the same block eleme~t divided by a BR. cPiNQ-±878
- Fhxed a bug with \ha2iµiÍt°nlvoÚt method where nonbreaiing spaces would be inserted inaorrectly. #TINY-1868
- Fized a bug whÂre the toolbar of the i~liÓ](efitor would not be visible in some scenarios. #TINY-1862
- Fixed a bug where removing dhe editÔr wËile more than one lÔt˘fmos4Iln 4b open would throw an error. #TINY-1845
- Fixed a bug wiere the menubutton would be"Úen`ered k, Ùop of the menu id ÑxUxu}˜xyr| fidn't have enough height. #TINY-1678
- Fixed a bug with the annotations api where0ajnotqtkng)ckllapsed selectiony°cIyrl‡!pg≈f\ÂEs. #TBS-2449
- Fixed a bug where wbr elements were being transformed into whitespace when usang the Paste Pluginßc p;Ûd'@Aa†tep< setting. #GH-4638
- Fixed a bug where the Search and Replace didn't replace spakes correct,y,$#GH-4632
